...
if __name__ == '__main__':
    ...
    dino_pca = torch.load(config['pca']['fp'], map_location=config['device'])
...
#    plt.show()
    ...
    for gi in tqdm.tqdm(range(len(gridmap_ts))):
        metadata_fp = os.path.join(args.run_dir, args.gridmap, '{:06d}_metadata.yaml'.format(gi))
        pcl_fp = os.path.join(args.run_dir, args.pcl, '{:06d}.npy'.format(pcl_idxs[gi]))
        image_fp = os.path.join(args.run_dir, args.image, '{:06d}.png'.format(image_idxs[gi]))
        ...
        dino_img = dino(img)[0]
        dino_img_norm = dino_img.view(-1, dino_img.shape[-1]) - dino_pca['mean'].view(1,-1)
        dino_img_pca = dino_img_norm.unsqueeze(1) @ dino_pca['V'].unsqueeze(0)
        dino_img = dino_img_pca.view(dino_img.shape[0], dino_img.shape[1], -1)

        # project pcl into dino image
