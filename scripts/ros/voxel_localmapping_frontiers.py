...
# from vfm_voxel_interfaces.srv import UpdatePrototype, UpdatePrototypeResponse
...
from physics_atv_visual_mapping.frontier_estimation.LRNAH_inference import LRNAHInferenceNode
...
class VoxelMappingNode:
    def __init__(self, config):
        ...
        self.do_frontier_estimation = "frontier_estimation" in config.keys()

        if self.do_frontier_estimation:
            rospy.loginfo("doing frontier estimation")
            self.frontier_estimator = LRNAHInferenceNode(config)
    ...
    def setup_ros_interface(self, config):
        ...
        # self.goal_sub = message_filters.Subscriber(config["waypoints"]["topic"], Pose)
        self.goal_sub = rospy.Subscriber(config["waypoints"]["topic"], PoseStamped, self.handle_goal)
        self.goal_pub = rospy.Publisher("pred_wpt", PoseStamped, queue_size=10)
        ...
        self.frontier_viz_pub = rospy.Publisher("frontier_viz", MarkerArray, queue_size=10)

        subs = [self.pcl_sub] + [self.image_subs[k] for k in self.image_keys]# + [self.goal_sub]
    ...
    def handle_update_prototypes(self, req):
        ...
        if not isinstance(trav_ptypes_block, TraversabilityPrototypesBlock):
            rospy.logwarn('not using trav ptypes!')
            return resp
        ...
        ## check that dimsize matches
        if len(req.data) != ptype_data.shape[-1]:
            rospy.logwarn('{} ptype shapes dont match (req: {} data: {})!'.format(
                req.id,
                len(req.data),
                ptype_data.shape[-1]
            ))
    ...
    def add_prototype_to_voxel_mapper(self, id, is_obstacle, modality, ptype):
        ...
        Args:
            ...
            is_obstacle: whether the prototype is obstacle
            ptype: the actual prototype data
        Returns:
            None

        This method will add an additional channel to the image pipeline and voxel mapper
        """
        with self.map_lock:
            trav_ptypes_block = self.image_pipeline.blocks[-1]
            ...
            new_voxel_data = torch.zeros(
                ...
                self.localmapper.voxel_grid.features.shape[1] + 1
            ).to(self.device)

            #update feat dim of mapping pipelines
            self.localmapper.n_features += 1
            if self.do_terrain_estimation:
                self.terrain_estimator.voxel_n_features += 1
    ...
    def preprocess_inputs(self):
        ...
        for img_key in self.image_keys:
            ...
            img = torch.as_tensor(img).unsqueeze(0).permute(0, 3, 1, 2).to(self.device)
            ...
            tf_veh_to_world_msg = self.get_tf("world", pc_frame, img_stamp)
            if tf_veh_to_world_msg is None:
                return None
            ...
            veh_to_world = tf_msg_to_htm(tf_veh_to_world_msg).to(self.device)
            # print("V2H", veh_to_world)
        ...
        rot = odom_to_vehicle_htm[:3,:3]

        if self.goal_msg is not None:
            goal_xy = [self.goal_msg.pose.position.x, self.goal_msg.pose.position.y]
        else:
            goal_xy = None

        # rospy.loginfo_throttle(5.0, 'colorized {}/{} points'.format(feature_pcl.features.shape[0], feature_pcl.pts.shape[0]))

        return {
            "pos": pos,
            "rot": rot,
            "veh2world": veh_to_world,
            "goal": goal_xy,
            # "pcl": pcl_in_odom,
            "images": images,
    ...
    def make_gridmap_msg(self, bev_grid):
        ...
        rospy.loginfo_throttle(5.0, "time to flatten layer {}: {}".format(i, accum_time))
        # add dummy elevation
    ...
    def make_img_msg_numpy(self, dino_img, img_key):
        viz_img = dino_img * 255
        img_msg = self.bridge.cv2_to_imgmsg(viz_img.astype(np.uint8), "rgb8")
        img_msg.header.stamp = self.image_data[img_key]['message'].header.stamp
        return img_msg
    ...
    def spin(self, event):
        ...
        with self.map_lock:
            ...
            if res:
                ...
                frontier_res = None
                if self.do_frontier_estimation and res["goal"] is not None:
                    
                    frontier_res = self.frontier_estimator.run(res)
                    now = time.perf_counter()
                    viz_msg = frontier_res['wpts_viz']
                    viz_msg.stamp = self.image_data['image_front']['message'].header.stamp
                    viz_msg.frame_id = 'crl_rzr/map'
                    self.frontier_viz_pub.publish(viz_msg.to_rosmsg())

                    pred_wp = frontier_res["pred_wp"]
                    pred_wp_msg = self.goal_msg
                    pred_wp_msg.pose.position.x = pred_wp[0]
                    pred_wp_msg.pose.position.y = pred_wp[1]
                    self.goal_pub.publish(pred_wp_msg)
                    # print('*********', time.perf_counter() - now)
                ...
                for i, img_key in enumerate(self.image_keys):
                    ...
                    elif self.do_frontier_estimation and frontier_res is not None:
                        score_img = frontier_res['overlays'][i]
                        # print(score_img.shape, score_img.min(), score_img.max())
                        msg = self.make_img_msg_numpy(score_img, img_key)
                    else:
                        score_img = feat_img
                        msg = self.make_img_msg(score_img, img_key)
